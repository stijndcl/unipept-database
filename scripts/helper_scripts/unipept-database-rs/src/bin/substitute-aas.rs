...
fn main() -> Result<()> {
    ...
    let sequences_infile = open_read(&args.sequences).context("Error opening sequences input file")?;
    ...
    for prefix in 'A'..='Z' {
        ...
        loop {
            ...
            let id_parsed = id.parse().with_context(|| format!("Error parsing {id} as an integer"))?;
